...
import { DeviceChip } from "./DeviceInfo";
...
const NoteIcon = chakra(ChatBubbleLeftEllipsisIcon as any, { baseStyle: { w: 3, h: 3 } });
...
type UsersTableProps = {
  ...
  onSelectUsers?: (usernames: string[]) => void;
} & TableProps;
export const UsersTable: FC<UsersTableProps> = ({ selectedUsers = [], onSelectUsers, ...props }) => {
  const {
    filters,
    users: usersData,
    onEditingUser,
    ...
    <Box id="users-table" overflowX={{ base: "unset", md: "unset" }}>
      ...
      >
        <Thead zIndex="docked" position="relative">
          <Tr>
            {onSelectUsers && (
              <Th position="sticky" top={{ base: "unset", md: top }} w="40px" px={2}>
                <Checkbox
                  colorScheme="primary"
                  isChecked={users.length > 0 && users.every((u) => selectedUsers.includes(u.username))}
                  isIndeterminate={selectedUsers.length > 0 && !users.every((u) => selectedUsers.includes(u.username))}
                  onChange={(e) => {
                    if (e.target.checked) {
                      onSelectUsers(users.map((u) => u.username));
                    } else {
                      onSelectUsers([]);
                    }
                  }}
            ...
            users?.map((user, i) => {
              return (
                <motion.tr
                  ...
                  onClick={() => {
                    if (selectedUsers.length > 0 && onSelectUsers) {
                      const already = selectedUsers.includes(user.username);
                      onSelectUsers(already ? selectedUsers.filter((u) => u !== user.username) : [...selectedUsers, user.username]);
                    } else {
                      onEditingUser(user);
                    }
                  }}
                  style={{ background: selectedUsers.includes(user.username) ? "rgba(14,165,233,0.07)" : undefined }}
                >
                  {onSelectUsers && (
                    <Td w="40px" px={2} onClick={(e) => e.stopPropagation()}>
                      <Checkbox
                        colorScheme="primary"
                        isChecked={selectedUsers.includes(user.username)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            onSelectUsers([...selectedUsers, user.username]);
                          } else {
                            onSelectUsers(selectedUsers.filter((u) => u !== user.username));
                          }
                        }}
                      />
                    </Td>
                    ...
                    <VStack spacing={1} align="flex-start">
                      ...
                      {user.last_connection && (
                        <Box onClick={(e) => e.stopPropagation()}>
                          <DeviceChip conn={user.last_connection} />
                        </Box>
                      )}
                    </VStack>
                  </Td>
